...
class AugmentedEnv(MultiDirEnv):
    ...
    This is different from CustomEnv because we want to make a very
    clear separation between tasks that were augmented from train tasks
    and held-out eval tasks. So we store the tasks in two separate
    folders per env: train and eval.
    ...
    Since augmentation takes a while, it should be run once separately
