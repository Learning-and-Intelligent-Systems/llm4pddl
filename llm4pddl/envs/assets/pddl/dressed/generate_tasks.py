...
def _generate_dressing_problems(min_people: int, max_people: int,
                                ...
                                num_probs: int, rng: np.random.Generator) -> List[str]:
    """Generates problems from a distribution. max numbers are exclusive, so if you
    want exactly two people set min to 2 and max to 3."""
    problems = []
...
def _generate_dressing_problem(num_people: int, num_casual_events: int, 
    ...
    dresses = [f'dress{i}' for i in range(1, num_formal_in_dress+1+exs['ex_d'])]
    sweatpants = [f'sweatpants{i}' for i in range(1, num_casual_events+1+exs['ex_sp'])]
    sweatshirts = [f'sweatshirt{i}' for i in range(1, num_casual_events+1+exs['ex_ss'])]
    nicepants = [f'nice-pants{i}' for i in range(1, num_formal_in_suit+1+exs['ex_np'])]
    collaredshirt = [f'collared-shirt{i}' for i in range(1, num_formal_in_suit+1+exs['ex_cs'])]
    suitjacket = [f'suit-jacket{i}' for i in range(1, num_formal_in_suit+1+exs['ex_d'])]
    ...
    for i in range(num_casual_events, (num_casual_events
                                       + num_formal_in_dress
                                       + num_formal_in_suit)):
        goal_predicates.append(f'(attending-formal-event {persons[i]})\n              ')
    rng.shuffle(goal_predicates)
    for goal in goal_predicates: goals+=goal
    #making problem:
...
def test_generate_dressing_problems() -> None:
    ...
    prob1 = _generate_dressing_problems(2, 3, 2, 3, 0, 1, 0, 1, 0, 1, 1, rng)[0]
