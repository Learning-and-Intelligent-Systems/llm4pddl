...
Run experiments sequentially, not in parallel.
    ...
    python scripts/launch_local.py --config scripts/configs/example.yaml
...
from cluster_utils import DEFAULT_BRANCH, config_to_cmd_flags, \
...
def _main() -> None:
    ...
    for cfg in generate_run_configs(args.config):
        ...
        cmd_flags = config_to_cmd_flags(cfg)
        cmd = f"python llm4pddl/main.py {cmd_flags} &> {logfile}"
        cmds.append(cmd)
